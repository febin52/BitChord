    ...
    suspend fun submitPlayingNow(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val trackMetadata = """{"track_metadata":{"artist_name":"${escapeJson(song.artist)}","track_name":"${escapeJson(song.title)}",$releasePart"additional_info":{${durationPart}"position_ms":$positionMs,"submission_client":"Apple Music"}}}"""
    ...
    suspend fun submitFinished(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val trackMetadata = """{"listened_at":$listenedAtStart,"track_metadata":{"artist_name":"${escapeJson(song.artist)}","track_name":"${escapeJson(song.title)}",$releasePart"additional_info":{${durationPart}"start_ms":$startMs,"end_ms":$endMs,"submission_client":"Apple Music"}}}"""
