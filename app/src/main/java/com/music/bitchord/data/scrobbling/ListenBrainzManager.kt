    ...
    suspend fun submitFinished(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val artist = if (primaryArtistOnly) song.artist.primaryArtist() else song.artist
                val trackMetadata = """{"listened_at":$listenedAtStart,"track_metadata":{"artist_name":"${escapeJson(artist)}","track_name":"${escapeJson(song.title)}",$releasePart"additional_info":{${durationPart}"start_ms":$startMs,"end_ms":$endMs,"submission_client":"Apple Music"}}}"""
